...
export async function POST(request: NextRequest) {
  try {
    ...
    // 檢查同一管理員是否已經簽名過
    const existingSignatures = await prisma.overtimeSignature.findMany({
      where: {
        userId,
        signedBy,
        date: {
    ...
    await prisma.overtimeSignature.createMany({
      data: signatureData,
    })

    return NextResponse.json({ 
      message: `成功簽名 ${dates.length} 個日期的加班記錄`,
      signedDates: dates,
    })
  } catch (error) {
    console.error('加班簽名失敗:', error)
    return NextResponse.json({ error: '加班簽名失敗' }, { status: 500 })
  }
} 